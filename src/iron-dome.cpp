...
int IronDomeProgram::init(void) {
  ...
  GuiElement shootRocketButton(
      {SCREEN_WIDTH - 300, SCREEN_HEIGHT - 100, 300, 30}, "Shoot enemty rocket",
      [this]() { add_enemy_rocket(); });
  ...
  for (int i = 0; i < DEFENDER_ROCKET_NUM; i++) {
    ...
    for (int j = 0; j < enemyRockets.size(); j++) {
      ...
      if (diff_norma < 2) {
        ...
        // realeas a bunch of falling parts
        for (int count = 0; count < 6; count++) {
          float randomZSpeed = (rand() % 5) - 2.5;
          float randomXSpeed = (rand() % 5) - 2.5;
          float randomYSpeed = (rand() % 5) - 2.5;

          // Go into different directions randomly
          fallingParts.push_back((FallingPart){
              .pos = {defenderRockets[i].pos.x, defenderRockets[i].pos.y,
                      defenderRockets[i].pos.z},
              .velocity = {randomXSpeed, randomYSpeed, randomZSpeed}});
        }
      }
    ...
    if (!isSurrounding) {
      ...
      for (int i = 0; i < tanks.size(); i++) {
        BoundingBox mouse_box = {
            {std::min(mouse_surround_pos.start.x, mouse_surround_pos.end.x),
             std::min(mouse_surround_pos.start.y, mouse_surround_pos.end.y),
             std::min(mouse_surround_pos.start.z, mouse_surround_pos.end.z)},
            {std::max(mouse_surround_pos.start.x, mouse_surround_pos.end.x),
             std::max(mouse_surround_pos.start.y, mouse_surround_pos.end.y),
             std::max(mouse_surround_pos.start.z, mouse_surround_pos.end.z)}};
        bool is_collision =
    ...
    for (int i = 0; i < enemyRockets.size(); i++) {
      ...
      if (!is_already_assigned) {
        ...
        for (int z = 0; z < ironDomes.size(); z++) {
          ...
          double x_diff = curr_enemy_rocket.pos.x - curr_capsule.pos.x;
          double y_diff = curr_enemy_rocket.pos.y - curr_capsule.pos.y;
          double z_diff = curr_enemy_rocket.pos.z - curr_capsule.pos.z;
          double distance =
        ...
        for (int j = 0; j < DEFENDER_ROCKET_NUM; j++) {
          if (defenderRockets[j].ironDomeId == closest_capsule &&
            ...
            ironDomes[defenderRockets[j].ironDomeId].remaining_rockets--;
            break;
          }
        }
      }
...
void IronDomeProgram::draw() {
  ...
  BeginMode3D(camera);
  // Draw the falling parts
  for (FallingPart fp : fallingParts) {
    std::cout << "Z is " << fp.pos.z << std::endl;
    DrawCubeV((Vector3){fp.pos.x, fp.pos.y, fp.pos.z}, {0.8, 0.8, 0.8}, PURPLE);
  }
  //
  //
  ...
  for (int i = 0; i < enemyRockets.size(); i++) {
    EnemyRocket curr_enemy_rocket = enemyRockets[i];
    if (curr_enemy_rocket.hasFallen() || curr_enemy_rocket.isDestroyed)
      continue;
    ...
    // float percentage = (100 - curr_enemy_rocket.destroyedPercentage) /
    // 100.0f; d = d / ((100 - curr_enemy_rocket.destroyedPercentage) / 100.0f);
    ...
    float finalX =
        curr_enemy_rocket.velocity.x * arrivalTime + curr_enemy_rocket.pos.x;
    float finalZ =
        curr_enemy_rocket.velocity.z * arrivalTime + curr_enemy_rocket.pos.z;
...
void IronDomeProgram::updateFallingParts() {
  //
  for (FallingPart &fallingPart : fallingParts) {
    // Make them fall with gravity
    // std::cout << "Speed is " << fallingPart.velocity.y << std::endl;
    fallingPart.velocity.y = fallingPart.velocity.y - 9.8 / 60.0f;
    fallingPart.pos.y += fallingPart.velocity.y / 60.0f;
    fallingPart.pos.x += fallingPart.velocity.x / 60.0f;
    fallingPart.pos.z += fallingPart.velocity.z / 60.0f;
  }
}
...
float getDistance(Vector3 v1, Vector3 v2) {
  ...
  // calc square root(size z^2 + y^2 +x^2)
  float diff_norma = sqrtf(z_diff * z_diff + y_diff * y_diff + z_diff * z_diff);
  return diff_norma;
}
